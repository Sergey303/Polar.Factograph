...
| `FileSystemFogRecordReader` | `FDataService.FillDb`, `DAdapter.FillDb0` | Removes `|` from identifiers, maps record and predicate local names to `http://fogid.net/o/`, preserves `xml:lang`, converts `iisstore/@uri` to the `uri` field, accepts anonymous RDF resource records and targetless legacy `<delete/>` markers | Reads one record at a time; replaces the legacy shared `no ident` value with a deterministic unique skolem URI for anonymous resources and ignores a targetless delete because it cannot affect any identified resource |
...
Anonymous resource support applies only to normal RDF resource records. A `delete` without `rdf:about` and without `id` is ignored because it has no target. A `substitute` still requires both an old and a new identifier and fails validation when either side is absent.
